...
import SubscriptionCard from "@/components/mypage/subscription-card" // 🔹 구독 카드 재사용
...
export default function SubscriptionManagementPage() {
  ...
  const isSubscribed = subscription.status === "ACTIVE" // ACTIVE는 구독 활성화일 경우 NONE은 구독 안한 상태일 경우 

  // 결제 수단 관리 모달 open 여부 (구독 중일 때만 사용)
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-8">
          <h1 className="mb-2 text-2xl font-semibold">구독 관리</h1>
          ...
          {isSubscribed ? (
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.2fr)]">
              ...
              <section className="rounded-xl border bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      감정 기반 투자 인사이트를 풀로 활용할 수 있는 월 구독 상품이에요.
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  ...
                  >
                    결제 수단 관리 <span className="text-xs text-gray-400">(예정)</span>
                  </button>
                ...
                <p className="mt-4 text-xs leading-relaxed text-gray-400">
                  첫 달 이후 해지하지 않으면 매월 {subscription.price.toLocaleString()}원이 자동 결제돼요.
            ...
            //  구독 안 한 사용자 화면 (구독 시작 유도)
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1.2fr)]">
              {/* 왼쪽: 빈 상태 + 설명 */}
              <section className="rounded-xl border bg-white p-8 shadow-sm flex flex-col justify-center">
                <div className="mb-4">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    SentiStock 프리미엄을 구독하면 감정 추세 히스토리와 즐겨찾기 확장,
                    매수 알림 기능을 모두 사용할 수 있어요.
                  </p>
                </div>
                ...
                <p className="mb-4 text-xs text-gray-400">
                  구독은 언제든 해지할 수 있고, 해지하더라도 남은 기간 동안은 프리미엄 기능을 계속 이용할 수 있어요.
                </p>

                {/* 이 버튼은 오른쪽 SubscriptionCard의 모달을 그대로 써도 되고,
                    단순 안내용으로만 둬도 됨. 지금은 스크롤 위치만 유지용 더미. */}
                <p className="text-xs text-gray-400">
                  지금 프리미엄 구독을 시작해 보세요.
                </p>
              </section>

              {/* 오른쪽: 실제 구독 카드 재사용 (모달 포함) */}
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
              ...
              <p className="mb-4 text-xs text-gray-400">
                결제 수단 변경은 다음 결제 시점부터 적용돼요. 이미 결제된 금액은 환불되지 않습니다.
              </p>
