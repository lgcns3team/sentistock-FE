...
export default function KakaoCallbackPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log("[onboardingRequired]", data.onboardingRequired);
      ...
      localStorage.setItem("onboardingRequired", String(data.onboardingRequired));
        // 이동
      router.replace(data.onboardingRequired ? "/onboarding" : "/main-page");
      } catch (e) {
  ...
  return <div className="p-6">{error ? error : "카카오 로그인 처리 중..."}</div>;
}